...
function updateTimeOptions() {
  ...
  if (!sel || !sel.value) {
    timeSelect.innerHTML = '<option value="">-- Select pickup week first --</option>';
    return;
  }
  const tueLabel = sel.dataset.tueLabel;
  const wedLabel = sel.dataset.wedLabel;
  [
    { value: sel.dataset.tue + "|afternoon", text: tueLabel + " \u2014 Afternoon (2pm \u2013 5pm)" },
    { value: sel.dataset.tue + "|evening",   text: tueLabel + " \u2014 Evening (5pm \u2013 7pm)"   },
    { value: sel.dataset.wed + "|morning",   text: wedLabel + " \u2014 Morning (8am \u2013 11am)"  },
  ].forEach((o) => {
    const opt = document.createElement("option");
    opt.value       = o.value;
...
function updateSummary() {
  ...
  specialItems.forEach((item, i) => {
    ...
    html +=
      ...
      item.label +
      ' <button type="button" class="remove-special" data-index="' + i +
  ...
  if (!html) {
    summaryLines.innerHTML = '<div class="summary-empty">No items selected yet \u2014 add something above!</div>';
  } else {
    html +=
...
document.querySelectorAll("[data-sized]").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    sizedQty[key] = Math.max(0, sizedQty[key] + parseInt(btn.dataset.delta, 10));
...
document.querySelectorAll("[data-add-special]").forEach((btn) => {
  btn.addEventListener("click", () => {
    specialItems.push({
      label: "Mother\u2019s Day Box: " + btn.dataset.name,
...
function getNameError(val, label) {
  ...
  if (!/^[A-Za-z\s'\-]+$/.test(clean) || clean.replace(/[^A-Za-z]/g, "").length < 2)
...
function getPickupError(val)     { return val ? "" : "Please select a pickup week"; }
function getPickupTimeError(val) { return val ? "" : "Please select a pickup time"; }
...
function validateFname()      { const e = getNameError(document.getElementById("fname").value, "First name"); setFieldError("fname", e);      return !e; }
function validateLname()      { const e = getNameError(document.getElementById("lname").value, "Last name");  setFieldError("lname", e);      return !e; }
function validateEmail()      { const e = getEmailError(document.getElementById("email").value);              setFieldError("email", e);      return !e; }
function validatePhone()      { const e = getPhoneError(document.getElementById("phone").value);              setFieldError("phone", e);      return !e; }
function validatePickup()     { const e = getPickupError(pickupSelect.value);                                 setFieldError("pickup", e);     return !e; }
function validatePickupTime() { const e = getPickupTimeError(timeSelect.value);                               setFieldError("pickupTime", e); return !e; }

function hasItems() {
  return breadQty > 0 || Object.values(sizedQty).some((v) => v > 0) || specialItems.length > 0;
}
...
function checkValidity() {
  const ok =
    ...
    pickupSelect.value && timeSelect.value && hasItems()                &&
    document.getElementById("payAgree").checked;
  submitBtn.disabled = !ok;
}

document.getElementById("fname").addEventListener("input",    () => { validateFname();      checkValidity(); });
document.getElementById("lname").addEventListener("input",    () => { validateLname();      checkValidity(); });
document.getElementById("email").addEventListener("input",    () => { validateEmail();      checkValidity(); });
document.getElementById("phone").addEventListener("input",    () => { validatePhone();      checkValidity(); });
pickupSelect.addEventListener("change",                       () => { validatePickup();     checkValidity(); });
timeSelect.addEventListener("change",                         () => { validatePickupTime(); checkValidity(); });
...
async function handleSubmit(e) {
  ...
  const pickupText = pickupEl.options[pickupEl.selectedIndex]?.text || "Not selected";
  ...
  try {
    ...
    if (res.ok) {
      ...
      confHTML += '<div class="conf-total">Estimated Total: $' + total + "</div>";
      ...
      alert("Something went wrong submitting your order. Please try again or contact us directly.");
    }
  } catch {
    submitBtn.innerHTML = "\uD83D\uDD25 Submit My Order";
    submitBtn.disabled  = false;
